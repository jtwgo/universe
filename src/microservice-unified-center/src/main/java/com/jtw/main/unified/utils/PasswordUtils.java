...
public class PasswordUtils {
    ...
    public static String encryptBySha256(String content, String salt)
    {
        try
        {
            MessageDigest digest = MessageDigest.getInstance("SHA1");
            byte[] bytes = (content + salt).getBytes("utf-8");
            byte[] encryptBytes = digest.digest(bytes);
            BASE64Encoder base64Encoder = new BASE64Encoder();
            String encrypt = base64Encoder.encode(encryptBytes);
